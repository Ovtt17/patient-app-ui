...
interface SelectableDoctorCardProps {
  doctor: DoctorResponse;
}

const SelectableDoctorCard: FC<SelectableDoctorCardProps> = ({ doctor }) => {
  const { firstName, lastName, email, phone, gender, profilePictureUrl, medicalLicense, officeNumber, specialties, id } = doctor;

  const { setValue } = useFormContext();
  const selectedDoctorId = useWatch({ name: 'doctorId' });

  const isSelected = selectedDoctorId === id;

  const handleSelect = () => {
    setValue('doctorId', id, { shouldValidate: true, shouldDirty: true });
  };
            ...
            specialties.map((spec) => (
              <span
                ...
                className={cn(
                  ...
                  isSelected
                    ? "bg-blue-100 dark:bg-purple-800 text-blue-700 dark:text-purple-200"
...
export default SelectableDoctorCard;